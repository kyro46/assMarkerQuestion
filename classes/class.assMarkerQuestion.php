...
class assMarkerQuestion extends assQuestion
{
    ...
    {
        $markers["features"] = json_decode($this->geojson, true);
        foreach ($markers as $marker) {
            foreach ($marker["features"] as $m) {
                include_once "class.assAnswerMarkerQuestion.php";
                array_push($this->markerdata,
                    new assAnswerMarkerQuestion(
                        $m["properties"]["name"],
                        $m["properties"]["points_answer"],
                        $m["properties"]["points_position"],
                        json_encode($m["geometry"]["coordinates"])));
            }
        }
    }
...
//var_dump($inputLabel,$solutionarray[$rowSolution][0],$distance);
                if ($inputLabel == $solutionarray[$rowSolution][0]) {
                    /* got ya!
                     * - grant point - check
    ...
    public function setExportDetailsXLS($worksheet, $startrow, $active_id, $pass)
    {
        ...
        for ($passNr = 0; $passNr <= $maxpass; $passNr++) {
            ...
            //Col 1: passNr
            $worksheet->writeString($i, 0, "Pass " . ($passNr + 1));

            //Col 2-4: label and points
            for ($input = 0; $input < count($gradingarray); $input++) {
                $worksheet->writeString($i, 2, $gradingarray[$input]["InputLabel"]);
                $worksheet->writeNumber($i, 3, $gradingarray[$input]["GrantedPointsforLabel"]);
                $worksheet->writeNumber($i, 4, $gradingarray[$input]["GrantedPointsforPosition"]);
