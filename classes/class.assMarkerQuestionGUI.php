 ...
 */
class assMarkerQuestionGUI extends assQuestionGUI implements ilGuiQuestionScoringAdjustable, ilGuiAnswerScoringAdjustable
{
    /**
     * @var ilassMarkerQuestionPlugin    The plugin object
     */
    var $plugin = null;
    ...
    public function editQuestion($checkonly = FALSE)
    {
        ...
        $this->tpl->addJavaScript($this->plugin->getDirectory() . '/js/ol.js');
        $this->tpl->addJavaScript($this->plugin->getDirectory() . '/js/ol3-contextmenu.js');

        $this->tpl->addCss($this->plugin->getDirectory() . '/css/ol.css');
        $this->tpl->addCss($this->plugin->getDirectory() . '/css/ol3-contextmenu.css');
        $this->tpl->addCss($this->plugin->getDirectory() . '/css/contextmenu.css');

        // background-image
        $image = new ilImageFileInputGUI($this->plugin->txt("image"), 'imagefile');

        if ($this->object->getImageFilename() != "") {
            $image->setImage($this->object->getImagePathWeb() . $this->object->getImageFilename());
        }
        $image->setInfo($this->plugin->txt('info_image'));
        $form->addItem($image);
        $form->addCommandButton("updateImage", $this->plugin->txt('upload_image'));
    ...
    {
        $levenshtein = new ilNumberInputGUI($this->plugin->txt("levenshtein"), "levenshtein");
        ...
        // predefmarker
        /*$predefmarker = new ilCheckboxInputGUI($this->plugin->txt("predefmarker"), 'predefmarker');
        ...
        // showpolygons
        $showpolygons = new ilCheckboxInputGUI($this->plugin->txt("showpolygons"), 'showpolygons');
    ...
    {
        include_once "class.ilMarkerWizardInputQuestionGUI.php";
        $markerdata = new ilMarkerWizardInputQuestionGUI("", "markerdata", $this->plugin);
        $markerdata->setKeyName($this->plugin->txt('marker_name'));
        $markerdata->setValueName($this->plugin->txt('points_answer'));
        $markerdata->setValues($this->object->getMarkerData());
        $markerdata->setRequired(true);
        $form->addItem($markerdata);
    ...
    {
        $template = $this->plugin->getTemplate("output.html");
        ...
        $this->tpl->addJavaScript($this->plugin->getDirectory() . '/js/ol.js');
        $this->tpl->addJavaScript($this->plugin->getDirectory() . '/js/ol3-contextmenu.js');
        //$this->tpl->addJavaScript($this->plugin->getDirectory().'/js/contextmenu.js');
    ...
    {
        $template = $this->plugin->getTemplate("solution.html");
        // get the solution of the user for the active pass or from the last pass if allowed
        ...
        if ($user_solution[0]["value1"]) {
            // use previously created geojson from students value-table
            $geojsonToDoc = $user_solution[0]["value1"];
            $usrSolPoint = $user_solution[0]["value2"];
        } else {
            if ($show_correct_solution) {
                $geojsonToDoc = $this->object->getGeoJSON();
            } else {
        ...
        $template->setVariable("GEOJSON_TEXT", $geojsonToDoc);
        $template->setVariable("USR_SOL_POINT", $usrSolPoint);
        $template->setVariable("GRAPHICAL_OUTPUT", $graphicalOutput);
        $template->setVariable("IMG_RIGHT", " <img src='" . ilUtil::getImagePath("icon_ok.svg") . "' alt='" . $this->lng->txt("answer_is_right") . "' title='" . $this->lng->txt("answer_is_right") . "' /> ");
        $template->setVariable("IMG_WRONG", " <img src='" . ilUtil::getImagePath("icon_not_ok.svg") . "' alt='" . $this->lng->txt("answer_is_wrong") . "' title='" . $this->lng->txt("answer_is_wrong") . "' /> ");
        $template->setVariable("GEOJSON", $geojsonToDoc);
        $template->setVariable("IMAGE_PATH", $this->object->getImagePathWeb() . $this->object->getImageFilename());
    ...
    {
        // TODO: Implement writeQuestionSpecificPostData() method.
    }
    ...
    {
        // TODO: Implement getAfterParticipationSuppressionQuestionPostVars() method.
    }
    ...
    {
        // TODO: Implement getAggregatedAnswersView() method.
    }
    ...
    {
        // TODO: Implement writeAnswerSpecificPostData() method.
    }
    ...
    {
        // TODO: Implement getAfterParticipationSuppressionAnswerPostVars() method.
    }
}
