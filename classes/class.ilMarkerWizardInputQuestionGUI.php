 ...
 */
class ilMarkerWizardInputQuestionGUI extends ilTextInputGUI
{
     ...
     * @param    string $plugin ilPlugin
     */
    function __construct($a_title = "", $a_postvar = "", $plugin)
    {
        $this->plugin = $plugin;
    ...
    function setValue($a_value)
    {
        $this->values = array();
        if (is_array($a_value)) {
            include_once "class.assAnswerMarkerQuestion.php";
            if (is_array($a_value['key'])) {
                foreach ($a_value['key'] as $idx => $key) {
                    array_push($this->values, new assAnswerMarkerQuestion(
                        $key,
                        $a_value['value'][$idx],
                        str_replace(",", ".", $a_value['points_position'][$idx]),
                        $a_value['coordinates'][$idx]));
                }
            }
        }
    }
     ...
     * Set key maxlength.
     *
     * @param    integer $a_maxlength Key maxlength
     */
     ...
     * Set value maxlength.
     *
     * @param    integer $a_maxlength value maxlength
     */
    ...
    function checkInput()
    {
        ...
        if (is_array($foundvalues)) {
            ...
            if (is_array($foundvalues['key']) && is_array($foundvalues['value'])) {
                ...
                foreach ($foundvalues['points_position'] as $val) {
                    if ($this->getRequired() && (strlen($val)) == 0) {
                        $this->setAlert($lng->txt("msg_input_is_required"));
                        return FALSE;
                    }
    ...
    function insert($a_tpl)
    {
        ...
        $tpl = $this->plugin->getTemplate("tpl.prop_markerquestionwizardinput.html");
        $i = 0;
//        var_dump($this->values);
        foreach ($this->values as $value) {
            if (is_object($value)) {
                if (strlen($value->name)) {
                    ...
                    $tpl->setVariable("PROPERTY_VALUE", ilUtil::prepareFormOutput($value->name));
                    $tpl->parseCurrentBlock();
                }
                if (strlen($value->points_answer)) {
                    $tpl->setCurrentBlock("prop_value_propval");
                    $tpl->setVariable("PROPERTY_VALUE", ilUtil::prepareFormOutput($value->points_answer));
                    $tpl->parseCurrentBlock();
                }
                if (strlen($value->points_position)) {
                    $tpl->setCurrentBlock("prop_points_propval");
                    $tpl->setVariable("PROPERTY_VALUE", ilUtil::prepareFormOutput($value->points_position));
                    $tpl->parseCurrentBlock();
                }
                if (strlen($value->coordinates)) {
                    $tpl->setCurrentBlock("prop_positions_propval");
                    $tpl->setVariable("PROPERTY_VALUE", ilUtil::prepareFormOutput($value->coordinates));
                    $tpl->parseCurrentBlock();
                }
            }
        ...
        $tpl->setVariable("POINTS_TEXT", $this->plugin->txt('points_position'));
